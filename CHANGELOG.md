...
- Added `mathjax` YAML parameter to allow support for MathJax, used to write LaTeX expressions (upgraded to MathJax v3)
- Added `author` YAML parameter to allow specifying the author(s) of a post (displayed in post previews)
- Added explicit support for favicons - simply add a `favicon.ico` file to the root directory
- Footer now sticks to the bottom, even on short pages (CSS flexbox implementation)
- Added social network links for GitLab, Bluesky, WhatsApp, Untappd, Strava
- Support reddit social network link to either be a subreddit or a user
- Use CSS variables for better theme customization
- Changed Twitter icon to X (fa-x-twitter)
- Upgraded Font Awesome to 6.5.2
- Fixed tables not having a scroll bar when wider than the page (usually happened on mobile)
- Fixed bug where search results broke if a post title had a backslash
- Fixed bug where hovering over search results showed the text "{desc}"
- Pagination buttons: use nicer arrows, and don't show text on small screens
- Added `title-on-all-pages` config setting, that adds the website title to all page titles
- Add author name to RSS feed

### Breaking Changes to Note
- `google-scholar` social network link no longer requires the prefix `citations?user=`
- Social network footer links order can now be customized
